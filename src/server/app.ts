...
import { Alert, CrowdPrediction, Station } from "../types";
import {
...
import { generateOperationalAdvisory } from "./geminiService";

// In-memory active alerts store to support persistent acknowledgments during server runtime
const activeAlerts: Alert[] = MOCK_ALERTS.map((a) => ({ ...a }));
...
export function createExpressApp() {
  ...
  apiRouter.get("/network/overview", (req: Request, res: Response) => {
    try {
  ...
  apiRouter.get("/stations/:id", (req: Request, res: Response) => {
    try {
  ...
  apiRouter.get("/stations/:id/platforms", (req: Request, res: Response) => {
    try {
      const stationId = req.params.id.toLowerCase();
      const stations = getAllSimulatedStations();
      const station = stations.find((s) => s.id.toLowerCase() === stationId);
  ...
  apiRouter.get("/network/heatmap", (req: Request, res: Response) => {
    try {
      const stations = getAllSimulatedStations();
  ...
  apiRouter.get("/trains", (req: Request, res: Response) => {
    try {
  ...
  apiRouter.get("/trains/:id", (req: Request, res: Response) => {
    try {
  ...
  apiRouter.get("/predictions/15m", (req: Request, res: Response) => {
    try {
      const stations = getAllSimulatedStations();
  ...
  apiRouter.get("/predictions/:stationId", (req: Request, res: Response) => {
    try {
  ...
  // This guarantees exact routing regardless of whether the hosting environment
  // passes the '/api' prefix or rewrites it.
